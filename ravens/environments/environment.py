...
class Environment(gym.Env):
  ...
  def __init__(self,
    ...
    self.nerf_cams = cameras.RealSenseD415.get_nerf_camera_config(n_input_views=n_input_views)
  ...
  def add_object(self, urdf, pose, category='rigid'):
    ...
    if self.save_blender:
      if 'stand' not in urdf:
        print(os.path.join(self.assets_root, urdf))
        self.recorder.register_object(obj_id, os.path.join(self.assets_root, urdf))
  ...
  def reset(self):
    ...
    if self.save_blender:
      self.recorder.register_object(self.ur5, os.path.join(self.assets_root, UR5_URDF_PATH))
      base_id, base_urdf, body_id, body_urdf = self.ee.get_registration_info()
      self.recorder.register_object(base_id, base_urdf)
      self.recorder.register_object(body_id, body_urdf)
  ...
  def start_pb_rec(self):
    """Pybullet to Blender"""
    from ravens.utils.pyBulletSimRecorder import PyBulletRecorder
    self.save_blender = True
    self.recorder = PyBulletRecorder()

  def end_pb_rec(self, filename):
